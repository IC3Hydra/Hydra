...
contract Head {   
    ...
    function HYDRA_INIT(address a, address b) {
        ...
        Initialized1(msg_sender, ead, mc);

        externalAddress = ead;
        metaContract = mc;
    }

    event InFallback1(uint256 calldatasize, address msg_sender, uint256 msg_value, bytes4 sig, uint256 arg1);
    event Initialized1(address sender, address ead, address mc);
    event Balance1(address a, uint256 b);
    event ExternalCall1(address a, uint256 ret, bool success);
    event LogCallBack1(uint256 a, uint256 b);
    ...
    function() payable {
        ...
        InFallback1(callDataSize, msg_sender, msg_value, sig, arg1);
    ...
    // return 1 (discrepancy with head2)
    function _return_one_or_two() internal {
        assembly {
            let x := mload(0x40)
            mstore(x, 0x20)
            mstore(add(x, 0x20), 0x1)
            return(x, 0x40)
        }
    }
    ...
    function _external_call(bytes4 sig, uint256 num_calls, uint256 arg, uint256 val) internal {
        ...
        ExternalCall1(ead, ret_val, call_success);
    ...
    function _log_event() internal {
        // sha3 of "LogCallBack1(uint256,uint256)"
    ...
    // a "throw" discrepancy (head2 returns constant)
    ...
    // this one will be explicitly omitted from the metacontract
    function not_in_abi() {}
}